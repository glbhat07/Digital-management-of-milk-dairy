...
$res = mysqli_query($conn,"SELECT * FROM region WHERE reg_code = '$empreg'");
    ...
    <h5 id="farmer_details">
     <?php
        if (isset($_REQUEST['region']) && $_REQUEST['region'] != '') {
            ...
            $result = mysqli_query($conn,"SELECT * FROM milk WHERE(dates BETWEEN '$start' AND '$end') AND (card_no IN(SELECT card_no from farmer WHERE region = '$empreg') )");

            $result1 = mysqli_query($conn,"SELECT SUM(quantity) as quantity, dates FROM milk WHERE(dates BETWEEN '$start' AND '$end') AND (card_no IN(SELECT card_no from farmer WHERE region = '$empreg') ) GROUP BY(dates)");
...
<a id="print1" class="btn btn-success" >print </a>
        </div>
        </body>
